package com.chickenman.question2.mycalssload;

import java.lang.reflect.InvocationTargetException;
import java.lang.reflect.Method;
import java.util.Base64;

/**
 * description:
 * @author 吴瑾
 * @version 1.0
 * @date 2020-10-16 15:17
 */
public class MyClassLoad extends ClassLoader {

    public static void main(String[] args) {
        MyClassLoad myClassLoad = new MyClassLoad();
        try {
            Object o = myClassLoad.findClass("Hello").newInstance();
            Method hello = o.getClass().getMethod("hello");
            hello.invoke(o);
        } catch (ClassNotFoundException | InstantiationException | IllegalAccessException | NoSuchMethodException | InvocationTargetException e) {
            e.printStackTrace();
        }
    }

    @Override
    protected Class<?> findClass(String name) throws ClassNotFoundException {
        String base64 = "";
        byte[] decode = Base64.getDecoder().decode(base64);
        byte[] helloDecode = new byte[decode.length];
        for (int i = 0; i < decode.length; i++) {
            helloDecode[i] = (byte) (255- decode[i]);
        }
        return defineClass(name, helloDecode, 0, helloDecode.length);
    }

}
